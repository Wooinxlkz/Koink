...
const AVATAR_ANIMATION_TIMELINE_STORAGE_KEY = 'koink-avatar-animation-timeline-v1'
...
function App({
  ...
  useEffect(() => {
    // Previously wrote resolvedTheme straight to document.documentElement's
    // `.dark` class whenever not `embedded` — which is always, since our
    // usage (via Root.tsx) never sets `embedded`. That meant this internal
    // theme state (defaulting to the OS preference, independent of any
    // choice made in the app) would silently overwrite whatever the app's
    // own global theme toggle (App.tsx's HomeHeaderActions) had set,
    // every time the editor mounted. Koink has exactly one source of truth
    // for `.dark` now — the global toggle — so this no longer writes here.
  ...
  const handleDownload = async () => {
    ...
    downloadBlob(
      `koink-avatar-${entityPreset}-${exportSize}.svg`,
  ...
  const handlePngDownload = async () => {
    ...
    try {
      ...
      downloadBlob(`koink-avatar-${entityPreset}-${exportSize}.png`, png)
      return true
    } catch (error) {
  ...
  const handleGifDownload = async () => {
    ...
    try {
      ...
      downloadBlob(
        `koink-avatar-${entityPreset}-${animationName.trim() || 'animation'}-${exportSize}.gif`,
        gif
      )
  ...
  // Duplicated the global top bar's theme/language controls (and still had
  // a GitHub link besides) — this whole block is now redundant with
  // App.tsx's own top-level controls, which render in every mode already.
  const renderGlobalHeaderActions = () => null
