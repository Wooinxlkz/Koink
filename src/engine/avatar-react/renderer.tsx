...
import { InteractiveAvatar } from '../avatar-app/InteractiveAvatar'
import type { AvatarViewState } from '../avatar-app/InteractiveAvatar'
import { resolveNativeAvatarPreset } from './native-preset'
import { resolveAvatarFaceStyle } from '../avatar-app/avatarGeometry'
import { renderAvatarPngBlob, renderAvatarSvgSource } from '../avatar-app/savedAvatarPresets'
...
export const Avatar = forwardRef<AvatarHandle, AvatarProps>(function Avatar({
  ...
  const resolveRequestedClip = useCallback((requested: AvatarAnimationClip | AvatarAnimationRef) => {
    ...
    if (selected == null) throw new Error('Unknown Koink Avatar animation')
  ...
  const play = useCallback(async (
    ...
    try {
      ...
      const resolved = error instanceof Error ? error : new Error('Unable to play Koink Avatar animation')
  ...
  const setTimeline = useCallback((
    ...
    try {
      ...
      const resolved = error instanceof Error
        ? error
        : new Error('Unable to configure Koink Avatar animation timeline')
      callbacksRef.current.onError?.(resolved)
      throw resolved
    }
  ...
  const frameShadow = scene.camera.showFrameShadow
    ...
    : 'none'
  const mergedStyle = {
    '--koink-avatar-background': scene.camera.background === 'transparent'
      ? 'transparent'
  ...
  return (
    <div
      {...divProps}
      ref={containerRef}
      className={`koink-avatar ${resolvedTheme === 'dark' ? 'dark' : ''}${className == null ? '' : ` ${className}`}`}
