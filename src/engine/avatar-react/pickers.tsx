...
import type { AvatarAnimationClip, AvatarAnimationRef, AvatarDefinition } from '@koink/avatar-core'
...
export function AvatarAnimationPicker({
  ...
  return (
    <div
      {...divProps}
      className={`koink-avatar-picker koink-avatar-animation-picker${className == null ? '' : ` ${className}`}`}
    >
      {searchable
        ? <input
            aria-label={placeholder}
            className='koink-avatar-picker__search'
          ...
          />
        : null}
      <div className='koink-avatar-picker__grid' role='listbox' aria-label='Animations'>
        {visibleOptions.map(option => (
          <button
            aria-label={option.label}
            aria-selected={option.id === value}
            className='koink-avatar-picker__option'
          ...
          >
            {option.previewUrl == null
              ? <span className='koink-avatar-picker__fallback' aria-hidden='true'>▶</span>
              : <img alt='' aria-hidden='true' draggable={false} src={option.previewUrl} />}
          </button>
        ))}
        {visibleOptions.length === 0
          ? <span className='koink-avatar-picker__empty' role='status'>{emptyLabel}</span>
          : null}
      </div>
    </div>
  )
...
export function AvatarPresetPicker({
  ...
  return (
    <div
      {...divProps}
      className={`koink-avatar-picker koink-avatar-preset-picker${className == null ? '' : ` ${className}`}`}
    >
      {searchable
        ? <input
            aria-label={placeholder}
          ...
          />
        : null}
      <div className='koink-avatar-picker__grid' role='listbox' aria-label='Avatars'>
        {visibleOptions.map(option => (
          <button
            aria-label={option.label}
            aria-selected={option.id === value}
          ...
          >
            {option.previewUrl == null
              ? <Avatar
                  aria-hidden='true'
                  className='koink-avatar-picker__avatar-preview'
